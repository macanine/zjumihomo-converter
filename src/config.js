export const clash_config = {
  ...
  "dns": {
    ...
    // 名单以「绕过 mihomo 的进程也要能拿真实 IP」为准：本机/局域网、NTP、
    // Windows NCSI、STUN 与游戏主机 NAT、captive portal 检测。国内域名不逐条列，
    // 末尾的 geosite:cn 一并覆盖——旧模板里音乐/银行/运营商那一大段都因此删去。
    "fake-ip-filter": [
      "*",
      "+.lan",
      "*.localdomain",
  ...
  "proxies": null,
  // 策略组含一套为 ACL4SSR 预设预备的备用组（微软服务、电报信息、全球拦截……）。
  // 组装配置时会按「规则是否引用到」剪除没人用的组（config-builder.js 的
  // prune_unused_groups），内置规则下客户端里只会看到实际参与分流的组。
  "proxy-groups": [
    {
      "name": "🚀 节点选择",
      "type": "select",
